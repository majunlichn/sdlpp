...
{
    return stbi_is_hdr(filename.data());
}
...
bool ImageU8::Load(std::string_view filename, int channelCount, float gamma, float scale)
{
    ...
    if (m_data)
    {
        m_dataSize = size_t(m_width) * size_t(m_height) *
...
{
    int result = stbi_write_png(filename.data(), m_width, m_height, m_channelCount,
        m_data, m_width * m_channelCount);
...
{
    int result = stbi_write_jpg(filename.data(), m_width, m_height, m_channelCount, m_data, quality);
    return (result != 0); // returns 0 on failure and non-0 on success.
}
